...
from src.ui.n8n_dashboard import (
    render_crawl_tab,
    render_n8n_tab,
    render_schedule_tab,
    render_search_tab,
...
st.set_page_config(page_title="쇼핑몰 탐색 & 크롤러", layout="wide")
st.title("국내 쇼핑몰 탐색 & n8n 크롤러")

tab_image, tab_search, tab_crawl, tab_stats, tab_schedule, tab_guide = st.tabs([
    "이미지 검색",
    "상품 검색",
    "크롤 실행",
    "현황/통계",
...
# ── 탭 1: 기존 이미지 기반 검색 파이프라인 ────────────────────────────────────
with tab_image:
    st.caption("제품 이미지 1장으로 국내 쇼핑몰 후보를 수집/분류/검수합니다.")
    cfg = load_config()
